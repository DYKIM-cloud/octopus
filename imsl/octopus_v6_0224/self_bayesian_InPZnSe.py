...
from Analysis.AnalysisUV import calculatePL_Data
...
def overwriteSinglePreviousModel(modelpath, savepath, params, loss_obj):
    ...
    #peak_location.append(property_tuple['lambdamax'])
    #pv_ratio.appemd(property_tuple['p_v_ratio'])
...
if __name__ == "__main__":
    ## INPUT(실험값)
    params =[{'AddSolution=InP_Injectionrate':3000.0, 'AddSolution=ZnSe_Injectionrate': 3000.0, 'Heat=Temperature':240.0}]
    ## OUTPUT(실험값)
    data_dict=load_result(path = "InPZnSe_flow\\240C_30s.txt")
    ## path
    job_script_path="InPZnSe.json"
    load_model_path="check.pickle"  # 반드시 다른이름으로 할 것
    save_model_path = "check.pickle"  # 반드시 다른이름으로 할 것 - 덮어쓰고나면 복구를 못해요.
    
    #######################################################
    #######################################################
    uv_result,uv_dict=calculatePL_Data(uv_dict=data_dict, WavelengthMin=300, WavelengthMax=700, BoxCarSize=1, Prominence=0.001, PeakWidth=10)
