...
def smooth_test(rawSpectrum, box_size):
    box = np.ones(box_size)/box_size
    spectrum_smooth = np.convolve(rawSpectrum, box, mode='same')
    spectrum_smooth = spectrum_smooth.reshape((1,len(rawSpectrum)))
...
if __name__ == "__main__":
    ...
    ## OUTPUT(실험값)
    path = "smoothingtest.csv"
    param_dict, data_dict=load_multisample_csv(path)

    ## path
    job_script_path="InP_core.json"
    load_model_path = "0930core.pickle"  # 반드시 다른이름으로 할 것
    save_model_path = "0930core.pickle"  # 반드시 다른이름으로 할 것 - 덮어쓰고나면 복구를 못해요.
    ...
    #for j in range(1,len(param_dict)+1):
        ...
        #######################################################
        #######################################################
    ref_dict = data_dict['Sample_1']
    wavelengths = np.asarray(ref_dict["Wavelength"], dtype=float)

    refspectrum = np.asarray(ref_dict["RawSpectrum"], dtype=float)
    back_raw    = np.asarray(data_dict['Sample_2']["RawSpectrum"], dtype=float)
    meas_raw    = np.asarray(data_dict['Sample_3']["RawSpectrum"], dtype=float)

    # ---- smooth_Boxcar가 기대하는 2×N 배열로 변환 ----
    ref_spec_arr     = np.vstack([wavelengths, refspectrum])   # shape: (2, N)
    back_spec_arr    = np.vstack([wavelengths, back_raw])      # shape: (2, N)
    measure_spec_arr = np.vstack([wavelengths, meas_raw])      # shape: (2, N)

    # 지금은 배경/측정은 스무딩 없이 원본 사용 (위 2줄 주석 해제 시 교체)
    ref_spec = ref_spec_arr
    back_spec    = back_spec_arr
    measure_spec = measure_spec_arr

    # ---- 스무딩 (원래 주석 상태 유지 가능) ----
    ref_spec = smooth_Boxcar(rawSpectrum=ref_spec_arr, box_size=20)   # 2×N 반환
    back_spec = smooth_Boxcar(rawSpectrum=back_spec_arr, box_size=20)
    measure_spec = smooth_Boxcar(rawSpectrum=measure_spec_arr, box_size=20)
    ...
    absorbances = []
    diff = []
    for ref, measured, bgd in zip(ref_spec[1], measure_spec[1], back_spec[1]):
        try:
            if ref == 0 or (measured - bgd) == 0:
                absorbances.append(0.0)
                continue
            diff.append(measured - bgd)
            absorbances.append(np.log10(abs(ref / (measured - bgd))))
        except Exception:
            break
    diff_spec_arr = np.vstack([wavelengths, diff])
    diff_spec = smooth_Boxcar(rawSpectrum=diff_spec_arr, box_size=20)
    abs2 = []
    for ref, dif in zip(ref_spec[1], diff_spec[1]):
        try:
            if ref == 0 or dif == 0:
                abs2.append(0.0)
                continue

            abs2.append(np.log10(abs(ref / dif)))
        except Exception:
            break
    # ---- 결과 패킹 및 플롯 ----
    uv_dict = {
        'Wavelength': wavelengths,
        'RawSpectrum': absorbances
    }

    plt.plot(wavelengths[:len(abs2)], abs2)
    ...
    '''
    # Convert to DataFrame
    data_df = pd.DataFrame(absorbances, index=wavelengths, columns=[f'Sample_{i}'])
