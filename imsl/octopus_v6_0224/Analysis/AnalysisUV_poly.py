...
def sliced_return(uv_df= pd.DataFrame, i = float):
    for sample in uv_df.columns:
        ...
        poly_wl, poly_ab = smooth_polyfit(savgoled[0], savgoled[1], degree=19)
        sliced2 = getSliceSpectrum(rawSpectrum=(poly_wl, poly_ab), min_wl=420, max_wl=550)
        baseline = sliced2[1][-1]
        corrected = np.array([sliced2[0], sliced2[1] - baseline])
    return corrected
...
def calculateUV_Data_clean_csv(
    ...
    """
    UV 흡광도 스펙트럼에서 peak-valley ratio와 lambdamax를 계산한다.

    파이프라인:
      1) 균일 그리드 보간 (350–950 nm, 20000 pts ≈ 33.3 pts/nm)
      2) Boxcar 스무딩  box_size=250 → ~7.5 nm  (고주파 노이즈 제거)
      3) 절삭 360–700 nm
      4) Savitzky-Golay  window=51 pts → ~1.5 nm  (edge-preserving 보정)
      5) Polynomial fit  degree=19  (피크 형상 보존)
      6) 최종 절삭 420–550 nm  (최소 420 nm 보장)
      7) analysisPickVelly 로 피크 검출

    기존 'sliced3' 축 교환 트릭 제거:
      이전 코드는 흡광도 0–1 범위만 남기도록 축을 바꾸었는데,
      QD 피크가 있는 420–450 nm 구간의 흡광도가 1 을 초과하면
      그 구간 전체가 잘려 피크 미검출 원인이 되었음.

    Parameters:
        uv_df (pd.DataFrame): wavelength를 index로 하는 흡광도 DataFrame.

    Returns:
        (peak_velly_ratio, peak_value): 검출 실패 시 (0.0, 0.0)
    """
    for sample in uv_df.columns:
        x = uv_df.index.values
        y = uv_df[sample].values
        ...
        order = np.argsort(spectrum[0])
        spectrum = np.array([spectrum[0][order], spectrum[1][order]], dtype=float)
        x_eval = np.linspace(350, 950, 20000)          # 33.3 pts/nm
        trend = np.interp(x_eval, spectrum[0], spectrum[1])
        raw = np.array([x_eval, trend], dtype=float)

        # 2) Boxcar 스무딩 ~7.5 nm  (box_size=250 @ 33.3 pts/nm)
        #    기존 199(~6 nm)에서 소폭 강화 — 피크 형상 보존 우선
        smoothed = smooth_Boxcar(rawSpectrum=raw, box_size=250)

        # 3) 360–700 nm 절삭  (polyfit 적용 범위)
        sliced = getSliceSpectrum(rawSpectrum=smoothed, min_wl=360, max_wl=700)

        # 4) Savitzky-Golay ~1.5 nm  (window=51 pts @ 33.3 pts/nm, odd, polyorder=3)
        #    기존 11 pts(~0.33 nm)에서 소폭 강화 — 국소 노이즈 스파이크만 제거
        savgoled = selective_noise_correction(
            sliced, noise_range=(360, 700), window_length=51, polyorder=3
        )

        # 5) Polynomial fit  degree=19  (피크 형상 보존, 기존 유지)
        poly_wl, poly_ab = smooth_polyfit(savgoled[0], savgoled[1], degree=19)
        ...
        #plt.show()
        # 7) 피크 검출  (sliced3 축 교환 제거 → 전체 420–550 nm 범위 사용)
        #    기존: sliced3 = getSliceSpectrum((sliced2[1],sliced2[0]), 0, 1)
        #    → 흡광도 > 1 구간(420–450 nm) 삭제로 피크 미검출 유발 → 제거
        peak_velly_ratio, peak_value = analysisPickVelly(
...
def calculateUV_Data_clean_csv2(
    uv_df: pd.DataFrame,
    gauss_sigma_nm: float = 3.0,
    bg_wl_min: float = 560.0,
    bg_wl_max: float = 700.0,
    peak_wl_min: float = 420.0,
    peak_wl_max: float = 510.0,
    prominence: float = 0.007,
    plot: bool = False,
) -> tuple:
    """
    calculateUV_Data_clean_csv 와 완전히 다른 방식의 UV 흡광도 전처리.

    ── 파이프라인 ──────────────────────────────────────────────────────
      1) 균일 그리드 보간  350–950 nm, 20 000 pts (≈ 33.3 pts/nm)
      2) Gaussian 스무딩  sigma = gauss_sigma_nm (기본 3 nm)
         · 고주파 스파이크 제거 / FFT 제거 → 링잉 없음
      3) 산란 배경 피팅  A_bg(λ) = a·(λ_ref/λ)^n
         · 장파장 560–700 nm 에 피팅; 신호 ≈ 0 이면 520–640 nm fallback
      4) 잔류 스펙트럼  resid = max(smoothed – A_bg, 0)
      5) 잔류에서 find_peaks  (420–510 nm, prominence 기준)
      6) 검출 시 잔류에 Gaussian 피팅 → 정밀 lambdamax + 정밀 amplitude
         · loss_comparison.py make_spectrum 처럼 bg + single-Gaussian 구조 재구성
      7) 재구성 스펙트럼  clean(λ) = A_bg(λ) + Gaussian(λ; amp, center, σ)
         · "noise를 강하게 스무딩하되, peak 부분은 스무딩하지 않는" 구조
         · 배경: power-law fit (완전 smooth)
         · 피크: Gaussian fit (형상 보존, 노이즈 제거)
      8) pv_ratio = (peak_ab – base_ab) / (valley_ab – base_ab)
         · 모두 재구성 스펙트럼 기준
         · 단, 배경이 거의 0 인 샘플은 y_smooth 기준 fallback

    Returns:
        (peak_velly_ratio, peak_value) — 미검출 시 (0.0, 0.0)
    """
    from scipy.optimize import curve_fit as _curve_fit

    PTS_PER_NM = 20000 / 600.0          # ≈ 33.3 pts/nm (350–950 nm 기준)
    ...
    for sample in uv_df.columns:
        ...
        # 2) Gaussian 스무딩 — 고주파 노이즈 스파이크 제거 (피크 감도 유지)
        y_smooth = gaussian_filter1d(y_interp, sigma=gauss_sigma_nm * PTS_PER_NM)

        # 3) 산란 배경 피팅: A_bg(λ) = a·(λ_ref/λ)^n
        #    - 1차: 560–700 nm (QD 피크 없는 장파장, n 자유)
        #    - Fallback 1: 520–640 nm (신호 약할 때)
        #    - Fallback 2: 420–510 nm, n ≤ 4 (물리 제약 Rayleigh/Mie)
        #      · 극저신호 샘플(≈ 0 배경)도 QD 어깨(shoulder) 감지 가능
        def _fit_bg(lo, hi, n_max=6.0):
            ...
            try:
                p, _ = _curve_fit(pl, xb, yb, p0=[float(yb[0]), 2.0],
        ...
        # Fallback 2: 피크 탐색 구간(420–510nm) 자체에 물리 제약(n ≤ 4) 배경 피팅
        if bg_full is None or bg_mean < 0.002:
        ...
        if pk_mask.sum() == 0:
            peak_velly_ratio, peak_value = 0.0, 0.0
            continue
        ...
        if len(peaks_i) == 0:
            ...
            # 6) 잔류에 Gaussian 피팅 → 정밀 lambdamax + amplitude
            win = 40.0
            ...
            try:
                ...
                po, _ = _curve_fit(_gauss, x_eval[wm], resid_full[wm],
            ...
            # 경계에 클램핑된 경우 → 420nm 이하에 실제 피크 → 거짓 검출로 처리
            if lambdamax <= peak_wl_min + 1.5:
                peak_velly_ratio, peak_value = 0.0, 0.0
                continue

            # 품질 필터: 4가지 조건 동시 만족해야 유효한 QD 피크로 인정
            #   1) Gaussian R² > 0.85   — 잔류가 실제 Gaussian 형태
            #   2) prominence > 0.010   — 배경 대비 충분한 진폭
            #   3) sig_fit ≥ 8 nm      — 물리적 최소 InP QD 피크 폭
            #   4) 실제 valley 존재 OR sig_fit < 20 nm (narrow shoulder 허용)
            ...
            quality_ok = (
                ...
                and prom_best > 0.010
                and sig_fit >= 8.0
                and (has_valley or sig_fit < 20.0)
            )
            if not quality_ok:
                peak_velly_ratio, peak_value = 0.0, 0.0
                continue

            # 7) pv_ratio: y_smooth 기준 (Gaussian fit은 lambdamax 정밀화에만 사용)
            peak_gauss_full = _gauss(x_eval, amp_fit, ctr_fit, sig_fit)

            idx_lm  = np.argmin(np.abs(x_eval - lambdamax))
            peak_ab = float(y_smooth[idx_lm])

            val_rng    = (x_eval >= lambdamax - 60.0) & (x_eval <= lambdamax - 10.0)
            valley_ab  = float(y_smooth[val_rng].min()) if val_rng.sum() > 0 \
                         else float(y_smooth[pk_mask][0])
            ...
            if denom <= 0.0:
                ...
                peak_velly_ratio, peak_value = (float(pv), lambdamax) if pv > 0.0 else (0.0, 0.0)
        ...
        if plot:
            ...
            # 상단: 원시 / 스무딩 / 배경 / 재구성 스펙트럼
            ax1.plot(x_eval, y_interp, color="#aaaaaa", linewidth=0.6,
                     alpha=0.6, label="Raw (interpolated)")
            ax1.plot(x_eval, y_smooth, color="#4c72b0", linewidth=1.2,
                     label=f"Gaussian smooth (σ={gauss_sigma_nm} nm)")
            ax1.plot(x_eval, bg_full,  color="#dd8452", linewidth=1.2,
                     linestyle="--", label="Background fit (power law)")

            if peak_value > 0:
                gauss_curve = _gauss(x_eval, amp_fit, ctr_fit, sig_fit)
                ax1.plot(x_eval, bg_full + gauss_curve, color="#55a868", linewidth=1.5,
                         linestyle="-.", alpha=0.8,
                         label=f"bg + Gaussian fit (λ={peak_value:.1f} nm)")
                ax1.axvline(peak_value, color="#c44e52", linewidth=1.0,
                            linestyle=":", label=f"λmax={peak_value:.1f} nm")
                ax1.scatter([peak_value], [float(y_smooth[np.argmin(np.abs(x_eval - peak_value))])],
                            color="#c44e52", zorder=5, s=50)

            ax1.axvspan(peak_wl_min, peak_wl_max, alpha=0.07, color="green",
                        label=f"Peak search [{peak_wl_min:.0f}–{peak_wl_max:.0f} nm]")
            ax1.set_ylabel("Absorbance")
            title = (f"Sample: {sample}  |  "
                     f"λmax={peak_value:.1f} nm  pv={peak_velly_ratio:.3f}"
                     if peak_value > 0 else f"Sample: {sample}  |  No peak detected")
            ax1.set_title(title)
            ax1.legend(fontsize=7, loc="upper right")
            ax1.set_xlim(350, 700)

            # 하단: 잔류 스펙트럼 (배경 제거 후)
            ax2.fill_between(x_eval, 0, resid_full, color="#8172b3", alpha=0.35,
                             label="Residual (smoothed − background)")
            ax2.plot(x_eval, resid_full, color="#8172b3", linewidth=1.0)
            ax2.axvspan(peak_wl_min, peak_wl_max, alpha=0.07, color="green")
            ax2.axhline(prominence, color="gray", linewidth=0.8, linestyle="--",
                        label=f"Prominence threshold ({prominence})")
            if peak_value > 0:
                ax2.axvline(peak_value, color="#c44e52", linewidth=1.0, linestyle=":")
            ax2.set_xlabel("Wavelength (nm)")
            ax2.set_ylabel("Residual absorbance")
