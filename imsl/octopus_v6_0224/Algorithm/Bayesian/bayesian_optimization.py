...
class BayesianOptimization(Observable):
    ...
    def _prime_subscriptions(self):
        if not any([len(subs) for subs in self._events.values()]):
            _MasterLogger = _get_default_logger(self._verbose)
    ...
    def array_like_constraints(self):
        ...
        # (e.g., replace 'AddSolution=InP_Injectionrate' before 'AddSolution=P_Injectionrate')
        sorted_keys_indices = sorted(enumerate(keys), key=lambda x: len(x[1]), reverse=True)

        array_like = []
        for constraint in self.constraints:
            tmp = constraint
            for idx, key in sorted_keys_indices:
    ...
    def get_constraint_dict(self):
        ...
        dicts = []
        funcs = []
        # Replace parameter names with x[idx] and evaluate via pandas safely.
        translated_constraints = self.array_like_constraints()
        for idx, constraint_expr in enumerate(translated_constraints):
            # pandas.eval needs the expression as a string; map into x vector.
            st = "def f_{0}(x): return pd.eval('{1}')\nfuncs.append(f_{0})".format(idx, constraint_expr)
            exec(st, {"pd": pd, "funcs": funcs})
...
class DiscreteBayesianOptimization(BayesianOptimization):
    ...
    def __init__(self, f, prange, random_state=None, verbose=2, constraints=[], targetName=""):
        ...
        self._gp = GaussianProcessRegressor(kernel=kernel,
                                            ...
                                            random_state=self._random_state)
        self._key_constraints = list(constraints)
        self.constraints = list(constraints)
        if self.constraints:
            self.constraints = self.array_like_constraints()
    ...
    def constrained_rng(self, n_points, bin=False):
        ...
        if len(self.constraints) > 1:
            # Multiple constraints: use the first constraint for constrained_rng
            # Other constraints will be checked during candidate validation
            pass
        elif len(self.constraints) == 0:
            raise ValueError("No constraints defined for constrained random number generator")
