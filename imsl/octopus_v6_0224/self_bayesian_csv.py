...
def createNewModel(configpath, savepath, params, loss_obj):
    ...
    #loss, dict = loss_obj.lambdamaxpvLoss_deriv(smooth_sigma=5, position_bandwidth=40.0,w_curv=0.6, w_pos=0.4)
    #loss, dict = loss_obj.lambdamaxpvLoss_decomp(smooth_sigma=4,peak_exclude_half=80.0, fwhm_range=550.0,peak_amp_ref=0.3,prominence=0.005,width_nm=10.0)
    print(f"loss={loss}")
    results=[loss]
...
if __name__ == "__main__":
    ...
    ## path
    job_script_path="InP_core_2.json"
    load_model_path = "0715_4.pickle"  # 반드시 다른이름으로 할 것
    save_model_path = "0715_4.pickle"  # 반드시 다른이름으로 할 것 - 덮어쓰고나면 복구를 못해요.
    sliced_dict = {}
    pv_list =[]
    ...
    for j in range(0,59):
        ...
        #save_to_fig(uv_df= data_df, i = i)
        sliced_array = sliced_return(uv_df= data_df, i = i)
        if j == 0:
            sliced_dict['x'] = sliced_array[0]
        sliced_dict[f'Sample {i}'] = sliced_array[1]
        ...
        #optimal_value, total_property_dict = loss_obj.lambdamaxpvLoss_deriv(smooth_sigma=5, position_bandwidth=40.0,w_curv=0.6, w_pos=0.4)
        #optimal_value, total_property_dict = loss_obj.lambdamaxpvLoss_decomp(smooth_sigma=4,peak_exclude_half=80.0, fwhm_range=550.0,peak_amp_ref=0.3,prominence=0.005,width_nm=10.0)
    ...
    for attr in ['samplingNum','res','constraints']:
        if hasattr(bo_obj.res, attr):
            val = getattr(bo_obj.res, attr)
