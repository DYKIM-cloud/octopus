...
def calculateUV_Data_clean_csv(
    ...
    """
    summary = []
    txt_path = Path('DB')/ "test_slice.txt"
    for sample in uv_df.columns:
        x = uv_df.index.values
        y = uv_df[sample].values
        ...
        #plt.show()
        sliced = getSliceSpectrum2(rawSpectrum=smoothed, min_wl=410, max_wl=720)
        savgoled = selective_noise_correction(sliced, noise_range=(430, 460), window_length=5, polyorder=3)
        ...
        #sliced = (cut_x,cut_y)
        poly_wl, poly_ab = smooth_polyfit(sliced[0], sliced[1], degree=20)
        #poly_wl, poly_ab = select_degree_cheb(sliced, deg_min=1, deg_max=30, kfold=5)
        #plt.plot(poly_wl,poly_ab)
        #plt.show()
        sliced2 = getSliceSpectrum2(rawSpectrum=(poly_wl, poly_ab), min_wl=420, max_wl=700)
        ...
        '''
        peak_velly_ratio, peak_value = analysisPickVelly(rawSpectrum=sliced2, prominence=0.015, width_threshold=5)
...
if __name__ == "__main__":
    ...
    for i in range(0,10):           # range(0,66)와 동일
        if i == 3:
            continue
