...
class Job(object):
    ...
    def __init__(self, jobScript):
        ...
        if self.algorithm_dict["model"] == "BayesianOptimization": 
        ...
        # load previous model
            ...
            model_path_list = self.algorithm_dict["modelPath"]
            if type(model_path_list)==list:
                self.Algorithm_obj=[]
                for idx, model_path in enumerate(model_path_list):
                    loaded_model=self.loadAIModel(model_path)
                    self.Algorithm_obj.append(loaded_model)
                    message="[jobID={0}] Algorithm, model : {1}".format(self.metadata_dict["jobID"], self.algorithm_dict["modelPath"][idx])
                    self.TaskLogger_obj.info(self.platform_name, message)
            elif type(model_path_list)==str:
                self.Algorithm_obj=self.loadAIModel(model_path_list)
                message="[jobID={0}] Algorithm, model : {1}".format(self.metadata_dict["jobID"], self.algorithm_dict["model"])
                self.TaskLogger_obj.info(self.platform_name, message)
    ...
    def execute(self, TaskScheduler_obj:object, TaskGenerator_obj:object):
        # run cycle
        ...
        if self.algorithm_dict["model"] != "Automatic":
            if 'modelPath' in self.algorithm_dict.keys():
            # if type(self.algorithm_dict["modelPath"])==list:
                ...
                # Modify metadata
                previous_model_length=len(self.algorithm_dict["modelPath"])
                totalExperimentNum=0
                for algorithm_obj in self.Algorithm_obj:
                    totalExperimentNum+=algorithm_obj.batchSize*self.algorithm_dict["totalCycleNum"]
                self.algorithm_dict["totalExperimentNum"]=totalExperimentNum
                ...
                for iter_num in range(self.algorithm_dict["totalCycleNum"]):
                    ...
                    for idx in range(previous_model_length):
                        self.TaskLogger_obj.info(self.algorithm_dict["modelPath"][idx], info_msg="batchSize={}".format(self.Algorithm_obj[idx].batchSize))
                        return_total_next_points, return_total_norm_next_points = self.Algorithm_obj[idx].suggestNextStep()# reaction space 이런 format 으로 바꾸기

                        total_next_points.extend(return_total_next_points)
                        total_norm_next_points.extend(return_total_norm_next_points)
                        print(total_next_points)
                    ...
                    self.TaskLogger_obj.current_platform_name="TaskGenerator-->generate process"
                    self.TaskLogger_obj.status="{}/{}:{}".format(self.TaskLogger_obj.currentExperimentNum, self.TaskLogger_obj.totalExperimentNum, self.TaskLogger_obj.current_platform_name) # in execution system
                    self.metadata_dict["jobStatus"]="{}/{}:{}".format(self.TaskLogger_obj.currentExperimentNum, self.TaskLogger_obj.totalExperimentNum, self.TaskLogger_obj.current_platform_name) # in execution system
                    total_process_template_list=[]
                    for batch_idx, total_next_point in enumerate(total_next_points):
                        hardware_next_point=ASLdiscreteBayesianOptimization.toHardwareParams(total_next_point) # Ratio/TotalFlowrate -> In/P Injectionrate (등록용 total_next_point는 그대로 둠)
                        each_process=TaskGenerator_obj.generateRecipe(self.process_dict, input_next_point=hardware_next_point)
                    ...
                    self.TaskLogger_obj.info("TaskGenerator", info_msg="Allocate all of process based on job script!")
                    ...
                    start_dir_name=time.strftime("%Y%m%d")
                    temp_return_result_list_to_db=copy.deepcopy(return_result_list_to_db)
                    for model_idx in range(previous_model_length):
                        optimal_value_list=[]
                        property_dict_list=[]
                        extract_total_next_points=[]
                        extract_total_norm_next_points=[]
                        for batch_idx in range(self.Algorithm_obj[model_idx].batchSize):
                            extract_total_next_points.append(total_next_points.pop(0))
                            extract_total_norm_next_points.append(total_norm_next_points.pop(0))
                            result_dict=temp_return_result_list_to_db.pop(0)
                            
                            Loss_obj=LossFunction(result_dict, self.Algorithm_obj[model_idx].lossTarget)
                            optimal_value, property_dict = getattr(Loss_obj, self.Algorithm_obj[model_idx].lossMethod)()
                            optimal_value_list.append(optimal_value)
                            property_dict_list.append(property_dict)
                            
                            self.TaskLogger_obj.info("Algorithm", info_msg="{} optimal_value : {}".format(model_idx, optimal_value))

                        dirname="USER/{}/DB/{}/{}/{}".format(self.metadata_dict["userName"], self.metadata_dict["subject"]+str(model_idx), start_dir_name, self.metadata_dict["modeType"])
                        each_cycle_num=int(len(self.Algorithm_obj[model_idx].res)/self.Algorithm_obj[model_idx].batchSize)
                        filename="{}_{}_{}_data".format(batch_finish_time, model_idx, each_cycle_num)
                        self.Algorithm_obj[model_idx].registerPoint(input_next_points=extract_total_next_points, norm_input_next_points=extract_total_norm_next_points, property_list=property_dict_list, input_result_list=optimal_value_list)
                        self.Algorithm_obj[model_idx].output_space(dirname+"/loss_norm", filename)
                        self.Algorithm_obj[model_idx].output_space_realCondition(dirname+"/loss_real", filename)
                        self.Algorithm_obj[model_idx].output_space_property(dirname+"/property", filename)
                    ...
                    start_dir_name=time.strftime("%Y%m%d")
                    for model_idx in range(previous_model_length):
                        each_cycle_num=int(len(self.Algorithm_obj[model_idx].res)/self.Algorithm_obj[model_idx].batchSize) # reaction space 도 체커 만들어서 같은 이름으로 함수 만들고 넣으면 될듯 
                        filename="{}_{}_{}_obj".format(batch_finish_time, model_idx, each_cycle_num)
                        self.savedModel(
                            model=self.Algorithm_obj[model_idx],
                            directory_path="USER/{}/SaveModel/{}/{}/{}".format(self.metadata_dict["userName"], self.metadata_dict["subject"]+str(model_idx), start_dir_name, self.metadata_dict["modeType"]), 
                            filename="{}_{}_{}_obj".format(batch_finish_time, model_idx, each_cycle_num))
                        self.TaskLogger_obj.info("Algorithm", info_msg="Save our model object, filename={}".format(filename))
                    ...
                    for idx, total_data_dict in enumerate(total_data_dict_list):
                        each_cycle_num=int(len(self.Algorithm_obj[idx].res)/self.Algorithm_obj[idx].batchSize)
                        dirname="USER/{}/DB/{}/{}/{}".format(self.metadata_dict["userName"], self.metadata_dict["subject"]+str(idx), start_dir_name, self.metadata_dict["modeType"])
                        filename="{}_{}_{}_data".format(batch_finish_time, idx, each_cycle_num)
                        # for code debugging
                ...
                for iter_num in range(self.algorithm_dict["totalCycleNum"]):
                    ...
                    self.savedModel(model = self.Algorithm_obj, directory_path="USER/{}/SaveModel/{}/{}/{}".format(self.metadata_dict["userName"], self.metadata_dict["subject"], start_dir_name, self.metadata_dict["modeType"]), 
            ...
            # Generate Recipe depending on task_sequence_list
            self.TaskLogger_obj.current_platform_name="TaskGenerator-->generate recipe"
            self.TaskLogger_obj.status="{}:{}".format(self.TaskLogger_obj.current_platform_name, self.algorithm_dict["totalExperimentNum"]) # in execution system
            self.metadata_dict["jobStatus"]="{}:{}".format(self.TaskLogger_obj.current_platform_name, self.algorithm_dict["totalExperimentNum"]) # in execution system
            # generate recipe
            total_process_template_list=[]
            for batch_idx, total_next_point in enumerate(total_next_points):
                ...
                self.__writeJsonFile("Test_{}_idx_{}.json".format(self.algorithm_dict["model"], batch_idx), each_process)
            print(':::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::')
            print(total_process_template_list)
            self.TaskLogger_obj.info("TaskGenerator", info_msg="Allocate all of process based on job script!")
...
if __name__=="__main__":
    model_path="USER/HJ/SaveModel/generate previous model (seed=2)/20231118/real/20231118_19_obj.pickle"
    try:
