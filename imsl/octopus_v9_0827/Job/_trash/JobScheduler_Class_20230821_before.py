...
from Master.Task.TCP import ParameterTCP, TCP_Class
...
class JobScheduler(JobScriptError, ParameterTCP):
    ...
    def updateNode(self, client_socket):
        ...
        command_byte="succeed to update node information:{}".format(self.ResourceManager_obj.task_hardware_info_dict)
        ...
        try:
            ...
            if user_name == total_job_queue[job_id_index].userName:
                ...
                if "Waiting" in popped_job_exec.TaskLogger_obj.status:
                    ...
                    self.ResourceManager_obj.refreshLocation(job_id)
    ...
    def qstat(self, client_socket):
        ...
        """
        column_names_list = ["userName", "jobTime", "jobID", "jobFileName", "currentExperimentNum", "totalExperimentNum", "status", "modeType"]
...
if __name__=="__main__":
    ...
    ssh = paramiko.SSHClient()
    ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy)
    PLATFORM_HOST="161.122.22.80"
    PLATFORM_USERNAME="user"
    PLATFORM_PWD="selfdriving!"
    ssh.connect(PLATFORM_HOST, username=PLATFORM_USERNAME, password=PLATFORM_PWD)
    print(f"{datetime.datetime.now().strftime('%Y-%m-%d %H:%M')} : SSH CONNECTED")
    stdin, stdout, stderr = ssh.exec_command("sh reboot_node_manager.sh")
