...
class LossFunction:
    ...
    def _preprocess_spectrum(self, wl_raw, rs_raw):
        ...
        :return: (pv_ratio, lambdamax, x_eval, y_smooth, wl_final, ab_final)
            pv_ratio   : peak-valley 비 (미검출 시 0.0)
            lambdamax  : 검출된 피크 파장 (미검출 시 0.0)
            x_eval     : 균일 파장 그리드 (20000 pts, Boxcar 스무딩 기준)
            y_smooth   : Boxcar-smoothed 스펙트럼 (x_eval 에 대응, SG/poly-fit 이전 단계)
            wl_final   : 420-550nm 절삭 + Savitzky-Golay + 19차 다항식 피팅까지 끝난
                         최종 곡선의 파장축 (analysisPickVelly 에 사용된 것과 동일)
            ab_final   : wl_final 에 대응하는 흡광도 (위와 동일한 최종 곡선)
        """
        ...
        return float(pv_ratio), float(lambdamax), x_eval, y_smooth, sliced2[0], sliced2[1]
    ...
    def lambdamaxpvLoss(self):
        ...
        측정 흡광도 = 배경 산란(scattering) 흡광도(1/λ^n 꼴, Rayleigh~Mie) + 성장한 QD의
        흡광도. QD가 자랄수록 이 배경 위에 흡수 신호(shoulder)가 '더해져서' 나타난다.
        배경만 있는 구간은 파장이 커질수록 계속 매끄럽게 줄어들 뿐이므로 기울기(dy)가
        어디서도 진짜 0 근처까지 가지 않는다 — 반면 QD 흡수가 실제로 더해진 지점은
        배경의 하강을 상쇄시켜 그 지점의 dy가 진짜로 0에 가까워지거나 국소적으로
        살짝 양수가 된다. 따라서 "하강 대비 상대적으로 덜 가파른 지점(prominence)"이
        아니라 "그 지점 자체의 절대적인 기울기가 실제로 0에 가까운지"를 봐야
        진짜 QD 신호와, 배경 곡선 자체의 자연스러운 굴곡(아직 QD 신호가 거의 없는
        경우)을 구분할 수 있다.
        ...
        ── 피크 미검출 → [-1.5, -0.3] ──
            420-550nm 최종(Savitzky-Golay + 19차 다항식 피팅) 곡선 전 구간의 1차 미분
            dy 를 계산하고, dy 의 국소 극대점(find_peaks) 후보들 중 **|dy| 가 가장
            0에 가까운(가장 평평한) 지점**을 숄더로 선택한다 (전체 point 기반,
            3개 고정 파장점 비교가 아님).

            combined(0~1)은 두 가지로 평가해 조합, 연속적으로 [-1.5,-0.3]에 선형 매핑:
              flatness = exp(-|dy_at_shoulder| / SHOULDER_DY_SCALE)
                         → 그 지점이 얼마나 '진짜로 평평한지'. |dy|=0(완전 평탄)일 때
                           1, |dy|가 커질수록(계속 하강 중일수록) 매끄럽게 0에
                           가까워진다. clip(0,1) 대신 지수 감쇠를 쓰는 이유:
                           선형 클리핑은 SHOULDER_DY_SCALE 을 넘는 순간 전부
                           flatness=0으로 뭉개버려서, |dy|가 서로 몇 배씩 차이나는
                           (즉 그래프 개형이 뚜렷이 다른) '성장 불충분' 샘플들이
                           전부 같은 loss로 뭉치는 문제가 있었음. 지수 감쇠는
                           |dy| 전 구간에서 계속 차등을 유지한다.
              position = exp(-0.5*((숄더 위치-target_lm)/40)^2)
                         → 숄더가 target_lm 에 얼마나 가까운지 (완만한 Gaussian 허용)
              combined = 0.6*flatness + 0.4*position ∈ [0,1]
              loss = -1.5 + 1.2*combined   (∈ [-1.5, -0.3])
              숄더 후보가 전혀 없으면(완전 단조 감소) combined=0 → -1.5 (최저)

            SHOULDER_DY_SCALE(=0.0002 abs/nm)는 이 데이터셋에서 실측으로 확인한
            경계값 — 실제 QD 성장이 보이는 샘플들은 숄더 지점의 |dy| 가 1e-5~7e-5
            수준(사실상 평평)인 반면, 아직 QD 형성이 충분치 않은 샘플들은 같은
            지점에서도 |dy| 가 1.7e-4 ~ 1.3e-3 로 뚜렷하게 남아있다. 지수 감쇠의
            특성 스케일(1/e 지점)을 그 경계 근방으로 캘리브레이션한 값.

        :return optimal_value (float): loss in [-1.5, 0]
        :return total_property_dict (dict): lambdamax, p_v_ratio 값
        """
        L_WORST        = -1.5  # 숄더 없음 (성장 신호 없음)
        L_NO_PEAK_CEIL = -0.3  # 숄더 최대 (검출 직전 수준) = 검출 최소값

        # 검출 성공 시 스케일 (최대 패널티 = 0.3 → 범위 [-0.3, 0])
        PEAK_SCALE = 0.3

        # ── 피크 미검출 시 '숄더' 평가 파라미터 ──────────────────────────
        SHOULDER_PROMINENCE_MIN = 1e-6    # find_peaks 후보 필터용 (수치 잡음 배제)
        SHOULDER_DY_SCALE       = 0.0002  # flatness 지수 감쇠의 특성 스케일 (abs/nm)
        SHOULDER_W_FLAT         = 0.6     # 숄더 평탄함(flatness) 가중치
        SHOULDER_W_POS          = 0.4     # target_lm 근접도(position) 가중치
        SHOULDER_POS_BANDWIDTH  = 40.0    # position score의 Gaussian 허용폭 (nm)
        ...
        for key, each in self.target_condition_dict.items():
            ...
            pv_ratio, lambdamax, x_eval, rs_smooth, wl_final, ab_final = self._preprocess_spectrum(
                wl_raw, rs_raw
            )
            ...
            if lambdamax > 0 and pv_ratio > 0:
                ...
                # 분모를 (target_pv - 1)로 정규화: pv_ratio 의 이론적 최솟값(1.0,
                # peak=valley)이 pv_loss=1(최대 패널티)에 정확히 대응하도록 스케일링.
                if pv_ratio >= target_pv:
                    pv_loss = 0.0
                else:
                    pv_loss = float(np.clip((target_pv - pv_ratio) / (target_pv - 1.0), 0.0, 1.0))

                optimal_value = -(lm_loss * w_lm + pv_loss * w_pv) * PEAK_SCALE

            else:
                # ── 피크 미검출: 420-550nm 최종 곡선 전체를 스캔해 숄더 탐지 ──
                dy = np.gradient(ab_final, wl_final)

                combined = 0.0
                peaks, props = find_peaks(dy, prominence=SHOULDER_PROMINENCE_MIN)
                if len(peaks) > 0:
                    # 후보들 중 |dy|가 가장 0에 가까운(가장 평평한) 지점을 선택
                    best         = int(np.argmin(np.abs(dy[peaks])))
                    shoulder_idx = peaks[best]
                    shoulder_wl  = float(wl_final[shoulder_idx])
                    dy_at_shoulder = float(abs(dy[shoulder_idx]))

                    flatness = float(np.exp(-dy_at_shoulder / SHOULDER_DY_SCALE))
                    position = float(np.exp(
                        -0.5 * ((shoulder_wl - float(target_lm)) / SHOULDER_POS_BANDWIDTH) ** 2
                    ))
                    combined = float(np.clip(
                        SHOULDER_W_FLAT * flatness + SHOULDER_W_POS * position, 0.0, 1.0
                    ))

                optimal_value = L_WORST + (L_NO_PEAK_CEIL - L_WORST) * combined
