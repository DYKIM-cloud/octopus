...
class ASLdiscreteBayesianOptimization(DiscreteBayesianOptimization):
    ...
    def _getNormalizedCondition(self, real_next_points):
        ...
        for _, next_point in enumerate(real_next_points):
            ...
            for chemical, rangeList in self.prange.items():
                new_value[chemical]=(float(next_point[chemical])-rangeList[0])/(rangeList[1]-rangeList[0]) # X' = (value - V_min)/(V_max - V_min)
    ...
    def _getRealCondition(self, normalized_next_points):
        ...
        for _, normalized_next_point in enumerate(normalized_next_points):
            new_value={}
            for chemical, rangeList in self.prange.items():
                lo, hi, step = rangeList
                raw_value = normalized_next_point[chemical]*(hi-lo)+lo
                new_value["{}".format(chemical)] = round(round(raw_value/step)*step, 6) # step 단위로 반올림 (정수 step만 가정한 round()는 소수 step(예: ratio)에서 값을 뭉갬)
            real_next_points.append(new_value)

        return real_next_points

    @staticmethod
    def toHardwareParams(point):
        """
        실행 직전 변환: {'AddSolution=Ratio': r, 'AddSolution=TotalFlowrate': Q, ...}
        -> {'AddSolution=In_Injectionrate': In, 'AddSolution=P_Injectionrate': P, ...}
        Ratio/TotalFlowrate 키가 없으면 원본을 그대로 반환 (다른 파라미터 구성의 job에서도 안전하게 재사용 가능).
        Ratio/TotalFlowrate 키는 In/P로 교체되며 제거된다 (TaskGenerator의 Collector 모듈이
        키 이름에 "AddSolution"이 포함된 값을 전부 유량으로 합산하므로, 남겨두면 잘못 합산됨).
        """
        if "AddSolution=Ratio" not in point or "AddSolution=TotalFlowrate" not in point:
            return point
        p = dict(point)
        r = p.pop("AddSolution=Ratio")
        Q = p.pop("AddSolution=TotalFlowrate")
        P = Q / (1 + r)
        In = Q - P
        p["AddSolution=In_Injectionrate"] = round(In) # 펌프는 정수 단위(µL/min)만 받으므로 정수로 반올림
        p["AddSolution=P_Injectionrate"] = round(P)
        return p

    @staticmethod
    def fromHardwareParams(point):
        """
        등록/학습용 변환: {'AddSolution=In_Injectionrate': In, 'AddSolution=P_Injectionrate': P, ...}
        -> {'AddSolution=Ratio': r, 'AddSolution=TotalFlowrate': Q, ...}
        In/P 키가 없으면 원본을 그대로 반환.
        """
        if "AddSolution=In_Injectionrate" not in point or "AddSolution=P_Injectionrate" not in point:
            return point
        p = dict(point)
        In = p.pop("AddSolution=In_Injectionrate")
        P = p.pop("AddSolution=P_Injectionrate")
        p["AddSolution=Ratio"] = In / P
        p["AddSolution=TotalFlowrate"] = In + P
        return p
