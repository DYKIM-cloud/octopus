...
# ========= 사용자 설정 =========
PICKLE_PATH = "0825.pickle"   # BO 피클 경로
...
for r in bo.res:
    ...
    # 파생 피처 1) log(In/P)  (BO는 Ratio=In/P를 직접 최적화하므로 log만 취하면 됨)
    log_ratio = np.log(ratio)

    # 파생 피처 2) Reaction Time (s) = (V / (TotalFlowrate + extra)) * 60
...
# ========= 2) 새 4D GPR 학습 =========
# 원 _gp는 Ratio/TotalFlowrate 기준이므로, 파생 4D(log ratio, RT, Preheat, Heat)로 새로 학습
# 피처별 원 스케일 차이(RT: 수백~천, Temp: 수십, log ratio: ~1)가 커서 표준화 없이 학습하면
# Matern length_scale이 한쪽으로 붕괴(degenerate)해 SHAP이 특정 피처에만 쏠리는 문제가 생김 -> 표준화 후 학습
